...
/**
 * Verifies that two {@link CloudMockExtension} instances are fully independent — each binds its own
 * port, and stopping one does not affect the other.
 */
class CloudMockExtensionIsolationTest {

    @RegisterExtension static CloudMockExtension first = new CloudMockExtension();

    @RegisterExtension static CloudMockExtension second = new CloudMockExtension();
