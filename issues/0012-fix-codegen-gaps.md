...
- [x] **Finding 1 — Separate template files:** For each operation, the generator writes a dedicated
  ...
  file per operation").
- [x] **Finding 2 — Generator unit tests:** A `src/test` directory exists in `cloudmock-codegen`
...
- [x] **Finding 3 — Published-artifact coordinates in generated `build.gradle`:** The generated
...
- For Finding 2, use minimal hand-authored fixtures kept under `src/test/resources/fixtures/`. Two
  formats are covered: `test-service.smithy` (Smithy IDL, `awsJson1_1`) and `widget-service.json`
  (Smithy JSON AST, `awsJson1_0` + `aws.api#service` trait), matching the format published by AWS
  in api-models-aws. Do not use real AWS Smithy models as test fixtures.
